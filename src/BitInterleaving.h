...
void Deinterleave2(InterleavedT in, IntegerT &a, IntegerT &b) {
  for (int i = 0; i < sizeof(a) * 8; i++) {
    a |= ((in & (InterleavedT)1 << (2 * i)) >> i);
    b |= ((in & (InterleavedT)1 << ((2 * i) + 1)) >> (i + 1));
  }
}
...
void Deinterleave3(InterleavedT in, IntegerT &a, IntegerT &b, IntegerT &c) {
  for (int i = 0; i < sizeof(a) * 8; i++) {
    a |= ((in & (InterleavedT)1 << i * 3) >> (i * 2));
    b |= ((in & (InterleavedT)1 << (i * 3) + 1) >> ((i * 2) + 1));
    c |= ((in & (InterleavedT)1 << (i * 3) + 2) >> ((i * 2) + 2));
  }
}
...
InterleavedT Interleave4(IntegerT a, IntegerT b, IntegerT c, IntegerT d) {
  InterleavedT out(0);
  for (int i = 0; i < sizeof(a) * 8; i++) {
    out |= (a & (IntegerT)1 << i) << (i * 3);
    out |= (b & (IntegerT)1 << i) << ((i * 3) + 1);
    out |= (c & (IntegerT)1 << i) << ((i * 3) + 2);
    out |= (d & (IntegerT)1 << i) << ((i * 3) + 3);
  }
  return out;
}
...
void Deinterleave4(InterleavedT in, IntegerT &a, IntegerT &b, IntegerT &c,
                   IntegerT &d) {
  for (int i = 0; i < sizeof(a) * 8; i++) {
    a |= ((in & (InterleavedT)1 << (i * 4)) >> (i * 3));
    b |= ((in & (InterleavedT)1 << ((i * 4) + 1)) >> ((i * 3) + 1));
    c |= ((in & (InterleavedT)1 << ((i * 4) + 2)) >> ((i * 3) + 2));
    d |= ((in & (InterleavedT)1 << ((i * 4) + 3)) >> ((i * 3) + 3));
  }
}
...
void Interleave4_128Bit(uint64_t &msb, uint64_t &lsb, const IntegerT a,
  ...
  const size_t halfInputBitLength(sizeof(a) * 4);
...
void Deinterleave4_128Bit(const uint64_t msb, const uint64_t lsb, IntegerT &a,
  ...
  for (int i = 0; i < halfInputBitLength; i++) {
    ...
    b |= ((msb & (uint64_t)1 << ((i * 4) + 1)) >> ((i * 3) + 1)) << halfInputBitLength;
    c |= ((msb & (uint64_t)1 << ((i * 4) + 2)) >> ((i * 3) + 2)) << halfInputBitLength;
    d |= ((msb & (uint64_t)1 << ((i * 4) + 3)) >> ((i * 3) + 3)) << halfInputBitLength;
  }
}
